...
int main(void)
{
	...
	int *test_arr = (int *)malloc(length * sizeof(int));					// Min number of elements in the array is 5 
	int *sorted_arr = (int *)malloc(length * sizeof(int));					// and the max is MAX_NUMBER
	if (!test_arr || !sorted_arr)								
	{											// Checks to see if the pointer arrays have been 
	...
	for (int i = 0; i < length; i++)
	{
		...
		for (int j = 0; j < length; j++)
		{
			...
			{									// checks if it has already been moved to the sorted array
				...
				num_duplicates = 0;						// If it is smaller it becomes the new smallest_num 
			}									// and num_duplicates is reset
		}
		for (int k = 0; k < num_duplicates+1; k++)					// Copies the smallest num and the correct number of duplicates 
			sorted[i+k] = smallest_num;						// to the sorted array
		i += num_duplicates;
		num_duplicates = 0;
		previous_num = smallest_num;							// Assigns smallest_num to previous_num to keep track of which
	}											// numbers have been moved
	return;
}
